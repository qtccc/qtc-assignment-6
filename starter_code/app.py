...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.rand(N).reshape(-1, 1) # generate random values for X
    ...
    for _ in range(S):
        ...
        X_sim = np.random.rand(N).reshape(-1, 1)  # generate X values

        # TODO: Generate Y values with normal additive error (mean mu, variance sigma^2)
        Y_sim = mu + np.random.randn(N) * np.sqrt(sigma2)  # generate Y values
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if N <= 0 or S <= 0 or sigma2 < 0:
                raise ValueError("N and S must be positive integers, and sigma2 must be non-negative.")
